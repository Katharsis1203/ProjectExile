...
export default function InventoryPage({
  ...
  const effectiveSelectedItemId = visibleEntries.some(
    (entry) => entry.id === selectedItemId,
  )
    ? selectedItemId
    : (visibleEntries[0]?.id ?? null);
  const selectedEntry =
    visibleEntries.find((entry) => entry.id === effectiveSelectedItemId) ?? null;
  ...
  return (
    <div className="fixed inset-0 z-[120] flex items-start justify-center overflow-y-auto bg-[#101722]/50 px-3 pb-5 pt-3 backdrop-blur-[3px] sm:px-6 sm:pt-5">
      <section
        ...
        className="relative flex min-h-[min(690px,calc(100dvh-2rem))] w-[min(1020px,96vw)] flex-col overflow-hidden bg-[length:100%_100%] bg-center bg-no-repeat px-5 pb-6 pt-5 text-[#33291f] drop-shadow-[0_26px_55px_rgba(0,0,0,0.45)] sm:px-8 sm:pb-8 sm:pt-7"
              ...
              {visibleEntries.length > 0 ? (
                <div className="space-y-1.5">
                  {visibleEntries.map((entry) => {
                    const active = effectiveSelectedItemId === entry.id;
