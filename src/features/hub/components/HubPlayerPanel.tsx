...
import PortraitAvatar from "../../player/components/PortraitAvatar";
import PlayerResourceBars from "../../player/components/PlayerResourceBars";
import PlayerStatusEffects from "../../player/components/PlayerStatusEffects";
...
export default function HubPlayerPanel({
  ...
  return (
    ...
    >
      <div
        className="relative z-10 flex h-full w-full flex-col overflow-hidden bg-[length:100%_100%] bg-center bg-no-repeat"
      ...
      >
        <section className="relative flex min-h-0 flex-1 flex-col px-5 pb-3.5 pt-5 text-[#2b2b2b]">
          <header className="relative z-40 border-b border-[#6c5942]/14 pb-2.5">
            <h2 className="font-serif text-[22px] font-bold leading-tight text-[#2f281f]">
              {player.name}
            </h2>
        ...
        <div className="relative z-50 flex flex-col border-t border-[rgba(70,58,44,0.15)]">
          ...
          <HubActionButton
            title="Explore"
            image={getImageUrl("alt/journal-btn4.png")}
            disabled={disabled}
            onClick={onExplore}
          />
          <HubActionButton
            title="Life"
            image={getImageUrl("alt/character-btn4.png")}
            disabled={disabled}
            onClick={onLife}
          />
        </div>
